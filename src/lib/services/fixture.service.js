import { insertRows, filter, updateRow, deleteRow, readSheet, where } from "../excel/store.js";
import { SHEETS, GROUP_STAGE, groupLabel } from "../excel/schema.js";
import { genId, fullName } from "../utils.js";
import { fixtureParticipants } from "./registration.service.js";
import { getTournament } from "./tournament.service.js";

// ---------------------------------------------------------------------------
// Fixture generation engine.
// Supports Knockout, Round Robin and League + Knockout.
// ---------------------------------------------------------------------------

const KO_ROUND_NAMES = {
  64: "Round of 64",
  32: "Round of 32",
  16: "Round of 16",
  8: "Quarter Finals",
  4: "Semi Finals",
  2: "Final",
};

// Fixed left-to-right progression of knockout round names — used both to
// generate the full bracket up front and to know which round feeds which.
export const KO_ROUND_ORDER = ["Round of 64", "Round of 32", "Round of 16", "Quarter Finals", "Semi Finals", "Final"];

function roundName(slots) {
  return KO_ROUND_NAMES[slots] || `Round of ${slots}`;
}

function nextPowerOfTwo(n) {
  let p = 1;
  while (p < n) p *= 2;
  return p;
}

// Build in-memory player/team lookup maps ONCE per call, instead of querying
// the database for every single participant while building a bracket. Over a
// network database, resolving 8+ participants one row at a time is the
// difference between a bracket generating instantly and taking many seconds.
async function buildLookup() {
  const [players, teams] = await Promise.all([readSheet(SHEETS.Players), readSheet(SHEETS.Teams)]);
  const playerById = new Map(players.map((p) => [p.id, p]));
  const teamById = new Map(teams.map((t) => [t.id, t]));
  return {
    resolveName: (id) => {
      if (!id) return "TBD";
      const p = playerById.get(id);
      if (p) return fullName(p);
      const t = teamById.get(id);
      if (t) return t.name;
      return String(id);
    },
    sideName: (participant) => {
      if (!participant) return "BYE";
      if (participant.isTeam) return teamById.get(participant.id)?.name || "Team";
      return fullName(playerById.get(participant.id)) || "Player";
    },
  };
}

function shuffle(arr) {
  // Fisher–Yates with Math.random so each regeneration yields a fresh draw.
  const a = [...arr];
  for (let i = a.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [a[i], a[j]] = [a[j], a[i]];
  }
  return a;
}

function makeMatch(tournamentId, round, court, s1, s2, lookup, groupName = "") {
  return {
    id: genId("MTC"),
    tournamentId,
    round,
    groupName,
    court: court || "",
    matchDate: "",
    matchTime: "",
    side1Id: s1?.id || "",
    side2Id: s2?.id || "",
    side1Name: lookup.sideName(s1),
    side2Name: lookup.sideName(s2),
    set1: "",
    set2: "",
    set3: "",
    winnerId: !s2 && s1 ? s1.id : "", // auto-advance byes
    loserId: "",
    duration: "",
    status: !s2 && s1 ? "Walkover" : "Scheduled",
  };
}

// Knockout: build the FULL bracket up front, like a printed tournament sheet.
// Round 1 is seeded with real participants (padded with byes to the next
// power of two — byes auto-win immediately). Every later round (Quarter
// Finals, Semi Finals, Final, ...) is created as a "TBD vs TBD" placeholder;
// advanceKnockout() fills each one in as the round before it finishes.
function generateKnockout(tournamentId, participants, lookup) {
  const n = participants.length;
  const size = nextPowerOfTwo(n || 2);
  const shuffled = shuffle(participants);
  const byesNeeded = size - n;

  // Build round-1 pairing slots: each of the first `byesNeeded` seeds gets a
  // bye (auto-advances). The rest pair up against each other. This guarantees
  // a bye is never paired against another bye — which would otherwise create
  // a dead "BYE vs BYE" match that can never be won and blocks the bracket.
  const slots = [];
  let idx = 0;
  for (let i = 0; i < byesNeeded; i++) slots.push(shuffled[idx++], null);
  while (idx < n) slots.push(shuffled[idx++], shuffled[idx++]);

  const matches = [];
  let court = 1;
  for (let i = 0; i < slots.length; i += 2) {
    matches.push(makeMatch(tournamentId, roundName(size), `Court ${court}`, slots[i], slots[i + 1], lookup));
    court = (court % 4) + 1;
  }

  let remaining = size / 2;
  while (remaining >= 2) {
    const count = remaining / 2;
    for (let i = 0; i < count; i++) matches.push(placeholderMatch(tournamentId, roundName(remaining)));
    remaining = remaining / 2;
  }
  return matches;
}

// Fill in the next knockout round's placeholders wherever both feeder matches
// are decided (Completed or a Walkover bye). Idempotent and safe to call after
// every result — re-reads from the store each pass so a chain of byes (e.g.
// two walkovers back to back) resolves in a single call.
export async function advanceKnockout(tournamentId) {
  const t = await getTournament(tournamentId);
  // Group + Knockout ends in a real bracket too, so it advances the same way
  // once advanceGroupStage() has created it.
  if (!t || (t.format !== "Knockout" && t.format !== "Group + Knockout")) {
    return { stage: "n/a", advanced: 0 };
  }

  const lookup = await buildLookup();

  // The bracket is read ONCE. Each pass then works on the in-memory copy,
  // applying the same change it writes — a chain of byes still resolves in a
  // single call, but without re-reading every match from the database on
  // every pass (which was up to six full reads per recorded result).
  const all = (await where(SHEETS.Matches, { tournamentId }))
    .filter((m) => !m.groupName); // group fixtures are not part of the bracket

  const isDone = (m) => m && (m.status === "Completed" || m.status === "Walkover") && m.winnerId;
  const pending = [];
  let totalAdvanced = 0;

  for (let pass = 0; pass < KO_ROUND_ORDER.length; pass++) {
    const used = KO_ROUND_ORDER.filter((name) => all.some((m) => m.round === name));
    if (used.length < 2) break;

    let advancedThisPass = 0;
    for (let i = 0; i < used.length - 1; i++) {
      const curr = all.filter((m) => m.round === used[i]);
      const next = all.filter((m) => m.round === used[i + 1]);
      for (let k = 0; k < next.length; k++) {
        const nm = next[k];
        if (nm.side1Id && nm.side2Id) continue; // already filled
        const a = curr[2 * k];
        const b = curr[2 * k + 1];
        if (!isDone(a) || !isDone(b)) continue;
        const patch = {
          side1Id: a.winnerId,
          side2Id: b.winnerId,
          side1Name: lookup.resolveName(a.winnerId),
          side2Name: lookup.resolveName(b.winnerId),
        };
        Object.assign(nm, patch); // keep the in-memory bracket in step
        pending.push({ id: nm.id, patch });
        advancedThisPass++;
      }
    }
    totalAdvanced += advancedThisPass;
    if (!advancedThisPass) break;
  }

  // All the slot fills go out together rather than one round trip each.
  if (pending.length) {
    await Promise.all(pending.map((u) => updateRow(SHEETS.Matches, u.id, u.patch)));
  }
  return { stage: totalAdvanced ? "advanced" : "none", advanced: totalAdvanced };
}

// ---------------------------------------------------------------------------
// Group stage
// ---------------------------------------------------------------------------

// Split participants into balanced pools of ~TARGET_GROUP_SIZE. The number of
// pools is driven by the target size, then entrants are dealt out as evenly as
// possible, so 10 entrants become 4/3/3 rather than 4/4/2 — nobody plays a
// materially shorter schedule than their rivals.
export function splitIntoGroups(participants, targetSize = GROUP_STAGE.TARGET_GROUP_SIZE) {
  const n = participants.length;
  if (n < 2) return [];
  // Never make more pools than we could fill: with 3 entrants and a target of
  // 4 that is a single pool, not three pools of one.
  const count = Math.max(1, Math.min(Math.ceil(n / targetSize), Math.floor(n / 2)));
  const groups = Array.from({ length: count }, () => []);
  // Deal round-robin ("snake" not needed — the draw is already shuffled), which
  // balances sizes automatically: sizes differ by at most one.
  participants.forEach((p, i) => groups[i % count].push(p));
  return groups;
}

// Every pool plays its own internal round robin. Rounds are named
// "Group A - Round 1" so they read correctly anywhere that only has the round
// string, while groupName carries the pool for filtering and standings.
function generateGroupStage(tournamentId, participants, lookup) {
  const groups = splitIntoGroups(shuffle(participants));
  const matches = [];
  groups.forEach((members, gi) => {
    const name = groupLabel(gi);
    const inner = generateRoundRobin(tournamentId, members, lookup);
    for (const m of inner) {
      m.groupName = name;
      m.round = `${name} - ${m.round}`;
      matches.push(m);
    }
  });
  return matches;
}

// How many knockout slots a completed group stage feeds. Kept here so the
// fixture engine and the advance step can never disagree about the size.
export function knockoutSlotsForGroups(groupCount, perGroup = GROUP_STAGE.QUALIFIERS_PER_GROUP) {
  return groupCount * perGroup;
}

// Pair qualifiers so that group winners meet runners-up from a DIFFERENT pool
// (A1 v B2, B1 v A2, C1 v D2, D1 v C2 ...) — the standard draw. Two sides from
// the same pool can still meet later in the bracket, just never in round one,
// so nobody replays a group fixture immediately.
//
// `ranked` is an array (one entry per group, in group order) of that group's
// qualifiers, best first.
export function seedQualifiers(ranked) {
  const pairs = [];
  for (let i = 0; i < ranked.length; i += 2) {
    const a = ranked[i] || [];
    const b = ranked[i + 1];
    if (!b) {
      // Odd number of groups — this pool's qualifiers pair against each other
      // rather than being left without an opponent.
      for (let k = 0; k < a.length; k += 2) pairs.push([a[k], a[k + 1]]);
      continue;
    }
    // A1 v B2 and B1 v A2.
    const depth = Math.max(a.length, b.length);
    for (let k = 0; k < depth; k++) {
      const winner = a[k];
      const runnerUp = b[depth - 1 - k];
      if (winner || runnerUp) pairs.push([winner, runnerUp]);
    }
  }
  return pairs;
}

// Round Robin: everyone plays everyone (circle method).
function generateRoundRobin(tournamentId, participants, lookup) {
  const players = [...participants];
  if (players.length % 2 !== 0) players.push(null); // bye marker
  const n = players.length;
  const rounds = n - 1;
  const half = n / 2;
  const arr = [...players];
  const matches = [];
  for (let r = 0; r < rounds; r++) {
    let court = 1;
    for (let i = 0; i < half; i++) {
      const a = arr[i];
      const b = arr[n - 1 - i];
      if (a && b) {
        matches.push(makeMatch(tournamentId, `Round ${r + 1}`, `Court ${court}`, a, b, lookup));
        court = (court % 4) + 1;
      }
    }
    // Rotate, keeping first fixed.
    arr.splice(1, 0, arr.pop());
  }
  return matches;
}

// League + Knockout: a round-robin group stage. Knockout is generated later
// (after standings are known) via generateKnockoutFromQualifiers.
function generateLeaguePlusKnockout(tournamentId, participants, lookup) {
  return generateRoundRobin(tournamentId, participants, lookup);
}

// Placeholder for a not-yet-determined playoff match. Sides are filled in
// later by the playoff engine as league results come in, so the tree shows
// "TBD vs TBD" cards right away instead of appearing only once qualified.
function placeholderMatch(tournamentId, round, groupName = "") {
  return {
    id: genId("MTC"),
    tournamentId,
    round,
    groupName,
    court: "",
    matchDate: "",
    matchTime: "",
    side1Id: "",
    side2Id: "",
    side1Name: "TBD",
    side2Name: "TBD",
    set1: "",
    set2: "",
    set3: "",
    winnerId: "",
    loserId: "",
    duration: "",
    status: "Scheduled",
  };
}

// Page-playoff placeholders sit alongside the league stage: with 3+
// participants that's Qualifier 1 + Semi Final + Final; with exactly 2
// participants (their single round-robin match already decides it) just Final.
function generatePlayoffPlaceholders(tournamentId, participantCount) {
  if (participantCount === 2) return [placeholderMatch(tournamentId, "Final")];
  if (participantCount >= 3) {
    return [
      placeholderMatch(tournamentId, "Qualifier 1"),
      placeholderMatch(tournamentId, "Semi Final"),
      placeholderMatch(tournamentId, "Final"),
    ];
  }
  return [];
}

export async function generateFixtures(tournamentId) {
  const { withDeferredRankings } = await import("./ranking.service.js");
  return withDeferredRankings(() => generateFixturesInner(tournamentId));
}

async function generateFixturesInner(tournamentId) {
  const t = await getTournament(tournamentId);
  if (!t) {
    const err = new Error("Tournament not found");
    err.status = 404;
    throw err;
  }
  const participants = await fixtureParticipants(tournamentId);
  if (participants.length < 2) {
    const isDoubles = String(t.category || "").toLowerCase().includes("doubles");
    const err = new Error(
      isDoubles
        ? "Need at least 2 teams to generate fixtures — add or generate teams first"
        : "Need at least 2 players to generate fixtures — add players first"
    );
    err.status = 400;
    throw err;
  }

  // Regenerate: clear any existing matches for this tournament first so the
  // action is idempotent (the client confirms before overwriting). Player
  // stats/rankings are always fully recomputed from the Matches sheet by
  // recalculateRankings() below, so a plain delete here is enough — no
  // per-match revert bookkeeping needed.
  const existing = await where(SHEETS.Matches, { tournamentId });
  for (const m of existing) await deleteRow(SHEETS.Matches, m.id);

  const lookup = await buildLookup();
  let matches;
  switch (t.format) {
    case "Round Robin":
      matches = generateRoundRobin(tournamentId, participants, lookup);
      matches.push(...generatePlayoffPlaceholders(tournamentId, participants.length));
      break;
    case "League":
    case "League + Knockout":
      matches = generateLeaguePlusKnockout(tournamentId, participants, lookup);
      matches.push(...generatePlayoffPlaceholders(tournamentId, participants.length));
      break;
    case "Group + Knockout": {
      matches = generateGroupStage(tournamentId, participants, lookup);
      // The bracket is built for real once the pools finish and we know who
      // qualified; placeholders up front would need re-sizing anyway.
      break;
    }
    case "Knockout":
    default:
      matches = generateKnockout(tournamentId, participants, lookup);
      break;
  }

  await insertRows(SHEETS.Matches, matches);

  // Byes/walkovers already have a winner the moment they're created (there's
  // no opponent to score against, so they never go through recordResult) —
  // resync rankings now so they're immediately counted as a played win
  // instead of undercounting the player until the next real result.
  const { recalculateRankings } = await import("./ranking.service.js");
  await recalculateRankings();

  // Immediately resolve any bye chains so the bracket reflects auto-advanced
  // walkovers the moment it's generated, not only after the next result.
  if (t.format === "Knockout") await advanceKnockout(tournamentId);

  return matches;
}

// Build the knockout bracket from a finished group stage. No-op until every
// group match is decided, and idempotent afterwards — a bracket already
// created is never rebuilt, so re-entering a group result can't wipe it.
export async function advanceGroupStage(tournamentId) {
  const t = await getTournament(tournamentId);
  if (!t || String(t.format) !== "Group + Knockout") return { stage: "n/a", advanced: 0 };

  const all = await where(SHEETS.Matches, { tournamentId });
  const groupMatches = all.filter((m) => m.groupName);
  if (!groupMatches.length) return { stage: "none", advanced: 0 };

  // Bracket already exists (any match without a groupName) — nothing to do.
  if (all.some((m) => !m.groupName)) {
    await advanceKnockout(tournamentId);
    return { stage: "knockout", advanced: 0 };
  }

  const decided = (m) => m.status === "Completed" || m.status === "Walkover";
  if (!groupMatches.every(decided)) {
    return { stage: "groups", advanced: 0, message: "Group matches are still in progress" };
  }

  // Standings per pool, best first — same ranking rule as everywhere else
  // (wins, then NPR). Imported lazily: playoff.service imports this module.
  const { groupStandings } = await import("./playoff.service.js");
  const groups = await groupStandings(tournamentId);
  if (!groups.length) return { stage: "groups", advanced: 0 };

  const ranked = groups.map((g) => g.rows.slice(0, GROUP_STAGE.QUALIFIERS_PER_GROUP));
  const pairs = seedQualifiers(ranked).filter(([a, b]) => a || b);
  if (pairs.length < 1) return { stage: "groups", advanced: 0 };

  const lookup = await buildLookup();

  // An odd number of pools gives a qualifier count that isn't a power of two
  // (3 pools -> 6 qualifiers, but the bracket needs 8 slots). Pad by SPLITTING
  // the top pairings into single-side matches, which auto-advance as byes —
  // rather than emitting an empty slot, which would create a dead
  // "BYE vs BYE" match nobody can ever win and the bracket would stall on.
  const size = nextPowerOfTwo(pairs.length * 2);
  const byesNeeded = size - pairs.length * 2;
  const splitCount = byesNeeded / 2; // always a whole number: both terms are even
  const firstRound = [];
  pairs.forEach((pair, i) => {
    const [a, b] = pair;
    if (i < splitCount) {
      // Highest-seeded pairings are the ones broken up, so the byes go to the
      // strongest group finishers — the usual seeding convention.
      if (a) firstRound.push([a, null]);
      if (b) firstRound.push([b, null]);
    } else {
      firstRound.push([a, b]);
    }
  });

  const matches = [];
  let court = 1;
  for (const [a, b] of firstRound) {
    const s1 = a ? { id: a.id, isTeam: false } : null;
    const s2 = b ? { id: b.id, isTeam: false } : null;
    const m = makeMatch(tournamentId, roundName(size), `Court ${court}`, s1, s2, lookup);
    // makeMatch's sideName() assumes a player for isTeam:false; resolveName
    // handles a player OR a team id, so Doubles sides get their team name.
    if (s1) m.side1Name = lookup.resolveName(a.id);
    if (s2) m.side2Name = lookup.resolveName(b.id);
    matches.push(m);
    court = (court % 4) + 1;
  }

  let remaining = size / 2;
  while (remaining >= 2) {
    for (let i = 0; i < remaining / 2; i++) matches.push(placeholderMatch(tournamentId, roundName(remaining)));
    remaining = remaining / 2;
  }

  await insertRows(SHEETS.Matches, matches);

  const { recalculateRankings } = await import("./ranking.service.js");
  await recalculateRankings();
  await advanceKnockout(tournamentId);

  return { stage: "knockout", advanced: matches.length, qualified: pairs.flat().filter(Boolean).length };
}
