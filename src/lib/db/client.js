...
async function migrateColumns(client) {
  ...
  for (const { table, col } of missing) {
    ...
    try {
      ...
      // On a deploy, several instances can boot at once and all see the column
      // as missing, so more than one issues the ALTER. Whoever loses that race
      // gets "duplicate column name" — which means the column now exists, i.e.
      // exactly the state we wanted. Anything else is a real failure.
      if (!/duplicate column name/i.test(String(err?.message))) throw err;
      console.log(`[DB] migrated: ${table}.${col} already added by another instance`);
    }
  }
}
